...
namespace WTLib.DependencyInjection.Internal
{
    ...
    internal class Meta : IMeta
    {
        ...
        public Meta(Type interfaceType, Type type, Lifestyle lifestyle)
        {
            ...
            {
                throw new IocContainerResolverException("Does not support ValueType, type:" + type.Name);
            }
            ...
            if (injectConstructors.Length == 0)
            {
                ...
                if (grouped == null)
                {
                    throw new IocContainerResolverException("Type does not found injectable constructor, type:" + type.Name);
                }
                else if (grouped.Count() != 1)
                {
                    throw new IocContainerResolverException("Type found multiple ambiguous constructors, type:" + type.Name);
                }
                else
                {
            ...
            {
                throw new IocContainerResolverException("Type found multiple [Inject] marked constructors, type:" + type.Name);
            }
            ...
            // Methods, [Inject] Only not support async, async is very complex in compiler.
            this.InjectMethods = this.Type.GetRuntimeMethods().Where(x =>
            {
                if (x.GetCustomAttribute<InjectAttribute>(true) != null)
                {
                    if (x.GetCustomAttribute<AsyncStateMachineAttribute>(true) != null)
                        throw new IocContainerResolverException("Can't inject async method.");
                    return true;
                }
                return false;
            }).ToArray();
