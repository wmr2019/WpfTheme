...
namespace WTLib.DependencyInjection.Internal
{
    internal class CompilationContext
    {
        ...
        public IMeta GetMeta(Type type)
        {
            ...
            {
                throw new IocContainerResolverException("Type is not registered, type:" + type.FullName);
            }
        }
        ...
        public void EnterEmit(Type type)
        {
            if (circularReferenceChecker.Any(x => x == type))
            {
                ...
                throw new IocContainerResolverException("Found circular reference: " + string.Join(" -> ", circularReferenceChecker.Select(x => x.Name)));
            }
            else
            {
                circularReferenceChecker.Push(type);
